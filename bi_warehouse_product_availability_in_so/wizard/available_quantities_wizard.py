...
class AvailableQuantitiesWizard(models.TransientModel):
    ...
    def _onchange_stock_product_id(self):
        if self.stock_product_id:
            stock_quant_id = self.env['stock.quant'].search([('product_id','=',self.stock_product_id.id),('location_id.usage','=','internal')])
            product_details = [product.id for product in stock_quant_id]
            self.stock_location_id = [(6, 0, product_details)]
