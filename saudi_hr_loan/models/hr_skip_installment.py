...
class HrSkipInstallment(models.Model):
    ...
    def _check_date(self):
        ...
        for employee_id in employee_ids:
            skip_date = datetime.strptime(
                str(employee_id.date), DEFAULT_SERVER_DATE_FORMAT
            )
    ...
    def onchange_employee(self):
        """
        Onchange the value based on selected employee company
        """
        if self.employee_id:
    ...
    def confirm_request(self):
        ...
        """
        self.ensure_one()
        loan_operation_ids = self.env["hr.loan.operation"].search(
            [
                ("loan_id", "=", self.loan_id.id),
                ("state", "not in", ["refuse", "done"]),
                ("loan_operation_type", "=", "pay_loan_amount"),
            ]
        )
        if self.loan_id.is_loan_freeze:
            raise UserError(_("Please Unfreeze Your Loan."))
        if any(
            self.date.month == loan_operation.date.month
            and loan_operation.loan_operation_type == "pay_loan_amount"
    ...
    def refuse_request(self):
        ...
        if self.state == "confirm":
            due_date = datetime.strptime(
                str(self.loan_id.due_date), DEFAULT_SERVER_DATE_FORMAT
            )
        ...
        for rec in self:
            if rec.state not in ["draft", "cancel"]:
                raise ValidationError(
                    _(
                        "You cannot delete a request to skip installment which is in %s state."
                    )
                    % (rec.state)
                )
