...
class SaleOrderLine(models.Model):
    ...
    def price_unit_check(self):
        bi_unit_price = self._get_bi_unit_price()
        ...
        elif self.price_unit < bi_unit_price:
            warning_mess = {
                'title': _('Message'),
                'message': _("Unit Price must be greater than or equal to BI Unit Price."),
            }
            return {'warning': warning_mess}

    def _get_bi_unit_price(self):
        """Fetch the BI Unit Price from the bi_sale_pricelist module."""
        pricelist = self.order_id.pricelist_id
        if not pricelist:
            return 0.0
        bi_unit_price = pricelist.bi_unit_price  # Assuming bi_unit_price exists in the pricelist
