...
class StockPicking(models.Model):
    ...
    def _get_default_branch_id(self):
        """تحديد الفرع الافتراضي بدون قيود الصلاحيات"""
        if len(self.env.user.branch_ids.sudo()) == 1:
            sp_company = self.company_id if self.company_id else self.env.company
            branch_ids = self.env.user.branch_ids.sudo()
            branch = branch_ids.filtered(
                lambda branch: branch.company_id == sp_company)
            if branch:
                return branch
    ...
    @api.depends('sale_id', 'purchase_id')
    def _compute_branch_id(self):
        """تعيين الفرع بدون قيود الصلاحيات"""
        for record in self:
            record.branch_id = False
            if record.sale_id.branch_id:
                record.branch_id = record.sale_id.branch_id.sudo()
            if record.purchase_id.branch_id:
                record.branch_id = record.purchase_id.branch_id.sudo()
