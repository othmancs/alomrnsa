odoo.define("saudi_hr_exp_info.employee_private_info", function (require) {
    "use strict";
    ...
    publicWidget.registry.employee_private_qualification_info = publicWidget.Widget.extend({
        ...
        events: {
            ...
            'dragover #dropZone': 'highlightDropZone',
            'dragleave #dropZone': 'unhighlightDropZone',
            'drop #dropZone': 'handleFileDrop',
            'change .address_home_country_id': 'onChangeCountry',
            'click .remove_resume': 'onClickRemoveResume'
        },
        ...
        onChangeCountry: function (ev) {

            var addressCountry = $('.address_home_country_id').find(':selected').val();
            if (addressCountry) {
                rpc.query({
                    model: 'res.country.state',
                    method: 'search_read',
                    args: [[['country_id', '=', parseInt(addressCountry)]], ['id', 'name']]
                }).then(function (datas) {
                    var address_states = $('.address_home_state_id');
                    address_states.empty();
                    _.each(datas, function (data) {
                        address_states.append($("<option></option>")
        ...
        handleFileDrop: function (ev) {
            ...
            if (file.type === 'application/pdf') {
                $('#dropped_resume_filename')[0].value = file.name;
                const reader = new window.FileReader();
                reader.onload = function (ev) {
                    dropPDF.innerHTML = `<iframe src="${ev.target.result}" frameborder="0" style="width: 100%; height: 100%;"></iframe>`;
                    dropped_resume.value = ev.target.result
                };
                reader.readAsDataURL(file);
                dropPDF.style.display = 'block';
            } else {
                console.log('Unsupported file format. Only PDF files are allowed.');
            }
        },

        onClickRemoveResume: function (ev) {
            $('#dropped_resume').val('');
            $('#dropped_resume_filename').val('');
            $('#dropPDF').empty();
            $('#dropPDF').attr('style', '');
        },
