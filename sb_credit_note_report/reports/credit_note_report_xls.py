...
class CreditNoteReportXlsx(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, partners):
        ...
        # Filter Information
        branch_names = data['form']['branch_names']
        ...
        # Table Header
        table_header = ['اسم الفرع', 'رقم الارجاع', 'اسم البائع', 'اسم العميل', 'تاريح الارجاع', 'اجمالى الارجاعات',
                        'تكلفة الارجاعات']
        ...
        for rec in data['invoice']:
            branch_name = rec['branch_id'][1] if rec['branch_id'] and isinstance(rec['branch_id'], list) and len(
                rec['branch_id']) > 1 else ''
            created_by = rec['created_by_id'][1] if rec['created_by_id'] and isinstance(rec['created_by_id'],
                                                                                        list) and len(
                rec['created_by_id']) > 1 else ''
            partner_name = rec['partner_id'][1] if rec['partner_id'] and isinstance(rec['partner_id'], list) and len(
                rec['partner_id']) > 1 else ''
