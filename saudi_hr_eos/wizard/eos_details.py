...
class EOSDetails(models.TransientModel):
    ...
    compute_at_date = fields.Selection(
        [("current_date", "Current Date EOS"), ("specific_date", "At a Specific Date")],
        string="Compute EOS",
        help="Choose to analyze the current EOS or from a specific date in the past.",
        default="current_date",
    )
    to_date = fields.Date(
        "EOS at Date",
        help="Choose a date to get the EOS at that date",
    ...
    def calculate_eos_reporting(self, to_date):
        """
        Calculates End of Service (EOS) reporting for employees based on their
        duration of employment and contract details.

        :param to_date: For calculating EOS (End of Service) reporting for employees based on their
        duration of employment and salary information
        :return: Returns a collection of EOS details records that are created based on the calculations
        performed for each employee who has a valid `date_of_join`in the HR system.
        The function calculates various durations, total EOS amounts, and payable EOS amounts based on
        the employee's tenure and contract details. It then creates EOS detail records with the
        calculated values and returns a collection of these
        """
    ...
    def open_eos_details(self):
        ...
        action = {
            ...
            "search_view_id": self.env.ref(
                "saudi_hr_eos.view_hr_eos_details_filter"
            ).id,
        }
        return action
