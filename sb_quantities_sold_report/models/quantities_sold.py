    ...
    def generate_xlsx_report(self, workbook, data, partners):
        ...
        for obj in partners:
            ...
            domain = [('invoice_date', '>=', obj.date_start),
                      ...
                      # ('invoice_line_ids.product_id.categ_id.id','=',obj.product_category_id.ids),
                      ('branch_id','=',obj.branch_id.ids)
                      ]
            if len(obj.product_category_id) == 1 and obj.product_category_id.name == 'All':
                pass
            else:
                domain.append(('invoice_line_ids.product_id.categ_id.id', 'in', obj.product_category_id.ids))
            ...
            for branch in existing_branch:
                ...
                for product in existing_products:
                    ...
                    product_qty = sum(current_branch_lines.invoice_line_ids.filtered(lambda x: x.product_id == product).mapped('quantity'))
                    domain_all_qty = [
                              ('state', '=', 'posted'),
                              ('move_type', '=', 'out_invoice'),
                    ...
                    product_qty_all = sum(qty_all.invoice_line_ids.filtered(lambda x: x.product_id == product).mapped('quantity'))
                    domain_on_hand = [
                        # ('date','<',obj.date_start),
