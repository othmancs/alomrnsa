...
class BranchSalesComparison(models.TransientModel):
    ...
    def generate_pdf_report(self):
        ...
        if len(self.product_category_id) == 1 and self.product_category_id.name == 'All':
            domain.append(('invoice_line_ids.product_id.categ_id.parent_id.name', '=', 'All'))
        else:
            domain.append(('invoice_line_ids.product_id.categ_id.id', 'in', self.product_category_id.ids))
        ...
        for branch in existing_branch:
            ...
            for product in existing_products:
                ...
                product_qty_all = sum(
                    qty_all.invoice_line_ids.filtered(lambda x: x.product_id == product).mapped('quantity'))
                domain_on_hand = [
                    # ('date','<',obj.date_start),
