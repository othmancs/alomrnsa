    ...
    def do_full_return_refund(self):
        ...
        try:
            for picking in self.sale_id.picking_ids.filtered(
                ...
                return_picking_id, pick_type_id = return_wizard.with_context(
            ...
            for invoice in self.sale_id.invoice_ids.filtered(
                ...
                if (
                    ...
                    and invoice.amount_residual == invoice.amount_total
                ):
                    reversal_form = Form(
                    ...
                    reversal_move = reversal_form.save()
                if reversal_move:
                    action = reversal_move.reverse_moves()
                    if (
                        reversal_move.refund_method == "refund"
                        and action
                        and action["res_id"]
                    ):
                        self.env["account.move"].browse(action["res_id"]).action_post()
