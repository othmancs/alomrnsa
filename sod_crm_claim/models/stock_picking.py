...
class StockPicking(models.Model):
    ...
    def copy(self, default=None):
        return_location = (
            ...
            .sudo()
            .get_param("sod_crm_claim.return_location_id")
        )
        if self.env.context.get("return_location", False) and return_location:
            return_location_id = self.env["stock.location"].browse(int(return_location))
            return_warehouse_id = return_location_id.get_warehouse()
            default = dict(default or {})
            default["picking_type_id"] = return_warehouse_id.in_type_id.id
