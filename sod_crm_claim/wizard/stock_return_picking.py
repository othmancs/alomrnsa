...
class StockReturnPicking(models.TransientModel):
    ...
    def _prepare_move_default_values(self, return_line, new_picking):
        ...
        return_warehouse = (
            ...
            .sudo()
            .get_param("sod_crm_claim.return_warehouse_id")
        )
        if self.env.context.get("return_warehouse", False) and return_warehouse:
            warehouse_id = self.env["stock.warehouse"].browse(int(return_warehouse))
            vals.update(
                {
    ...
    def _onchange_picking_id(self):
        ...
        if self.env.context.get("return_warehouse_loc", False) and return_warehouse:
            warehouse_id = self.env["stock.warehouse"].browse(int(return_warehouse))
            self.location_id = warehouse_id.in_type_id.default_location_dest_id.id
        return res
