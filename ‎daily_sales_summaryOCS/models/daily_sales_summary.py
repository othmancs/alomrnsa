...
class DailySalesSummary(models.Model):
    ...
    #     # جمع جميع طرق الدفع وتجميع المتشابهة
    #     payment_methods_data = defaultdict(float)
    #     payment_methods_names = set()
    
    #     # 1. جمع طرق الدفع من الفواتير المدفوعة
    ...
    #             if payment.payment_method_line_id:
    #                 method_name = payment.payment_method_line_id.name or 'غير محدد'
    #                 # تجميع طرق الدفع التي تحتوي على "نقدي" تحت نفس الاسم
    #                 normalized_name = 'نقدي' if 'نقدي' in method_name else method_name
    #                 payment_methods_data[normalized_name] += payment.amount
    #                 payment_methods_names.add(normalized_name)
    
    #     # 2. جمع طرق الدفع من المدفوعات الواردة
    ...
    #     # ترتيب طرق الدفع مع وضع "نقدي" أولاً
    #     sorted_methods = sorted(payment_methods_names, key=lambda x: (x != 'نقدي', x))
    ...
    #     # إضافة أعمدة لطرق الدفع المجمعة
    ...
    #                     method_name = payment.payment_method_line_id.name or 'غير محدد'
    #                     normalized_name = 'نقدي' if 'نقدي' in method_name else method_name
    #                     method_totals[normalized_name] += payment.amount
    ...
    #         # حساب المقبوضات حسب طريقة الدفع (مجمعة)
    #         payment_method_totals = defaultdict(float)
    #         for payment in payments:
    #             if payment.payment_method_line_id:
    #                 method_name = payment.payment_method_line_id.name or 'غير محدد'
    #                 normalized_name = 'نقدي' if 'نقدي' in method_name else method_name
    #                 payment_method_totals[normalized_name] += payment.amount
            
    #         branch_total_receipts = sum(payment_method_totals.values()) - branch_cash_refunds + branch_partial_sales
    ...
    #         for method in sorted_methods:
    #             totals[f'method_{method}'] += method_totals.get(method, 0.0)
    ...
    #         # كتابة مبيعات كل طريقة دفع (مجمعة)
    #         for method in sorted_methods:
